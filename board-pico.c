...
static struct perflock_platform_data pico_perflock_data = {
...
static uint32_t camera_off_gpio_table[] = {
	...
	GPIO_CFG(pico_GPIO_CAM_D1V8_EN,	 0, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA),
	GPIO_CFG(pico_GPIO_CAM_A2V85_EN,	 0, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA),
	/* HTC_END */
};
...
static void pico_camera_vreg_config(int vreg_en)
{
	...
	if (vreg_en) {
		if(system_rev >= 0x1){ /* for XB board with 5M and later*/
			...
			/* A2V85 */
			gpio_set_value(pico_GPIO_CAM_A2V85_EN, 1);
			...
			/* D1V8 for 3M */
			gpio_set_value(pico_GPIO_CAM_D1V8_EN, 1);
			pr_info("enable D1V8\n");
			udelay(5);
		...
		if(system_rev >= 0x1){ /* for XB board with 5M and later */
			...
			/* A2V85 */
			gpio_set_value(pico_GPIO_CAM_A2V85_EN, 0);
			...
			/* D1V8 */
			gpio_set_value(pico_GPIO_CAM_D1V8_EN, 0);
...
struct msm_camera_device_platform_data pico_camera_device_data_rear = {
...
struct msm_camera_device_platform_data pico_camera_device_data_front = {
...
static struct msm_camera_sensor_info msm_camera_sensor_mt9t013_data = {
	...
	.pdata          = &pico_camera_device_data_rear,
...
static struct msm_camera_sensor_info msm_camera_sensor_ov9726_data = {
	...
	.vcm_pwd                = 1,
	.vcm_enable             = 0,
	.pdata                  = &pico_camera_device_data_front,
...
static struct platform_device *pico_camera_5M_devices[] __initdata = {
	&msm_camera_sensor_s5k4e1,
};

static struct platform_device *pico_camera_3M_devices[] __initdata = {
	&msm_camera_sensor_mt9t013,
};
#endif
...
static struct synaptics_i2c_rmi_platform_data pico_ts_synaptics_data[] = { /* Synatpics sensor */
	{
		.version = 0x3330,
		.abs_x_min = 0,
		.abs_x_max = 1100,
...
static struct i2c_board_info i2c_touch_device[] = {
	{
		...
		.platform_data = &pico_ts_synaptics_data,
...
static struct kobj_attribute pico_synaptics_virtual_keys_attr = {
	.attr = {
...
static struct attribute *pico_properties_attrs[] = {
	&pico_synaptics_virtual_keys_attr.attr,
	NULL
};
...
static void config_nfc_gpios(void)
{
	static uint32_t nfc_gpio_table[] = {
		GPIO_CFG(pico_GPIO_NFC_VEN, 0, GPIO_CFG_OUTPUT,
			GPIO_CFG_NO_PULL, GPIO_CFG_2MA),
		GPIO_CFG(pico_GPIO_NFC_DL, 0, GPIO_CFG_OUTPUT,
			GPIO_CFG_NO_PULL, GPIO_CFG_2MA),
		GPIO_CFG(pico_GPIO_NFC_INT, 0, GPIO_CFG_INPUT,
...
static struct pn544_i2c_platform_data nfc_platform_data = {
	...
	.irq_gpio = pico_GPIO_NFC_INT,
	.ven_gpio = pico_GPIO_NFC_VEN,
	.firm_gpio = pico_GPIO_NFC_DL,
	.ven_isinvert = 1,
	.boot_mode = 0,
};
...
static struct i2c_board_info pn544_i2c_boardinfo[] = {
	{
		...
		.irq = MSM_GPIO_TO_INT(pico_GPIO_NFC_INT),
	},
};
...
static void __init pico_init(void)
{
...
#ifdef CONFIG_PERFLOCK
	perflock_init(&pico_perflock_data);
#endif
	...
	platform_add_devices(pico_devices,
		...
		if(system_rev >= 0x1) /* for XB board */
		  platform_add_devices(pico_camera_5M_devices,ARRAY_SIZE(pico_camera_5M_devices));
		else /* for XA board */
		  platform_add_devices(pico_camera_3M_devices,ARRAY_SIZE(pico_camera_3M_devices));
#endif
	...
	if (board_build_flag() == 1) {
		for (i = 0; i < ARRAY_SIZE(pico_ts_synaptics_data);  i++)
			pico_ts_synaptics_data[i].mfg_flag = 1;
	}
